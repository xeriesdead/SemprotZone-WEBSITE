...
function isDoodStreamEmbedUrl(source?: string | null) {
  if (!source) return false;
  try {
    const url = new URL(source);
    return url.pathname.includes("/e/") && url.hostname.includes("dood");
  } catch {
    return false;
  }
}
...
function Watch({ watchlist, toggle }: { watchlist: ReturnType<typeof useWatchlist>; toggle: (id: string) => void }) {
  ...
  const doodStreamEmbed = isDoodStreamEmbedUrl(source);
  return <div className="cinema-grain min-h-[100dvh] bg-[#090d12] text-stone-100"><div className="mx-auto max-w-[1440px] pb-16 pt-5 md:px-8 md:pt-8"><div className="mb-5 flex items-center justify-between px-4 md:px-0"><button type="button" onClick={() => setLocation(`/title/${id}`)} className="inline-flex items-center gap-2 text-xs font-semibold text-stone-400 transition hover:text-amber-200" data-testid="button-back-title"><ArrowLeft className="h-4 w-4" /> <span className="hidden sm:inline">Back to title</span></button><Logo /><button type="button" onClick={() => setAdVisible((visible) => !visible)} className="text-xs text-stone-500 transition hover:text-amber-200" data-testid="button-toggle-ad">{adVisible ? 'Hide ad treatment' : 'Show ad treatment'}</button></div><div className="relative overflow-hidden rounded-md bg-[#151a21] shadow-[0_20px_80px_rgba(0,0,0,.38)]"><div className="aspect-video w-full">{source ? doodStreamEmbed ? <iframe src={source} title={content.type === 'series' && episode ? episode.title : content.title} className="h-full w-full border-0" allow="autoplay; fullscreen" allowFullScreen data-testid="doodstream-player" /> : <video ref={videoRef} controls autoPlay={false} poster={content.backdropUrl || content.posterUrl} className="h-full w-full" onPlay={() => setPlaying(true)} onPause={() => setPlaying(false)} data-testid="video-player"><source src={source} /></video> : <div className="flex h-full flex-col items-center justify-center bg-[radial-gradient(circle_at_center,#2b3036,#11161d_70%)] text-center"><Film className="mb-4 h-10 w-10 text-amber-300/70" /><h2 className="font-display text-2xl">Playback is being prepared.</h2><p className="mt-2 max-w-xs text-xs leading-5 text-stone-500">There is no video source attached to this reel yet. You can still explore the title and episodes.</p></div>}</div>{adVisible && <div className="absolute left-4 top-4 flex items-center gap-2 rounded bg-[#10151d]/85 px-3 py-2 text-[10px] font-mono-ui uppercase tracking-[.12em] text-stone-300 backdrop-blur"><span className="h-1.5 w-1.5 rounded-full bg-[#e47b67]" /> Ad-supported screening</div>}{source && !doodStreamEmbed && <div className="absolute bottom-12 left-4 flex items-center gap-2"><button type="button" onClick={() => { if (videoRef.current) { if (playing) videoRef.current.pause(); else void videoRef.current.play(); } }} className="flex h-9 w-9 items-center justify-center rounded-full bg-amber-300 text-[#11161d] transition hover:bg-amber-200" aria-label={playing ? 'Pause video' : 'Play video'} data-testid="button-player-play">{playing ? <span className="text-xs font-bold">Ⅱ</span> : <Play className="h-4 w-4 fill-current" />}</button><button type="button" onClick={() => setMuted((value) => !value)} className="flex h-9 w-9 items-center justify-center rounded-full bg-[#10151d]/75 text-stone-200 backdrop-blur transition hover:text-amber-200" aria-label={muted ? 'Unmute video' : 'Mute video'} data-testid="button-player-mute">{muted ? <VolumeX className="h-4 w-4" /> : <Volume2 className="h-4 w-4" />}</button></div>}</div><div className="mt-6 flex flex-col justify-between gap-5 border-b border-white/[.08] px-4 pb-7 sm:flex-row sm:items-start md:px-0"><div><p className="font-mono-ui text-[10px] uppercase tracking-[.17em] text-amber-300/80">{content.type === 'series' && episode ? `Season ${episode.season} · Episode ${episode.episode}` : 'Feature presentation'}</p><h1 className="mt-2 font-display text-3xl text-stone-100 md:text-4xl">{content.type === 'series' && episode ? episode.title : content.title}</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-stone-500">{content.type === 'series' && episode ? episode.description : content.description}</p></div><div className="flex shrink-0 gap-2"><WatchlistButton content={content} saved={watchlist.has(content.id)} onToggle={() => toggle(content.id)} light /><button type="button" onClick={() => { void navigator.clipboard?.writeText(window.location.href); }} className="flex h-11 w-11 items-center justify-center rounded-full border border-stone-500/50 text-stone-300 transition hover:border-amber-300/60 hover:text-amber-200" aria-label="Copy link" data-testid="button-share"><Share2 className="h-4 w-4" /></button></div></div>{content.type === 'series' && episodes.length > 1 && <div className="mt-8 px-4 md:px-0"><div className="mb-4 flex items-center justify-between"><h2 className="font-display text-xl">More episodes</h2><span className="text-[10px] font-mono-ui uppercase tracking-[.14em] text-stone-500">{episodes.length} in this season</span></div><div className="hide-scrollbar flex gap-3 overflow-x-auto pb-2">{episodes.map((item) => <Link href={`/watch/${content.id}?episode=${item.id}`} key={item.id} className={cx('min-w-[190px] rounded border p-2 transition', item.id === episode?.id ? 'border-amber-300/60 bg-amber-300/[.08]' : 'border-white/10 bg-white/[.025] hover:border-amber-300/40')} data-testid={`link-watch-episode-${item.id}`}><div className="aspect-video overflow-hidden rounded-sm bg-[#232931]"><MediaImage src={item.thumbnailUrl} alt={item.title} className="h-full w-full object-cover" /></div><p className="mt-2 truncate text-xs font-semibold">{item.title}</p></Link>)}</div></div>}</div></div>;
}
